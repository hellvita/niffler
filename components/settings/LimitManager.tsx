...
export function LimitManager() {
  ...
  const handleSetLimit = () => {
    ...
    setFormError(null);
    setLimit.mutate(
      { effectiveFromDate, amount: parsed },
      {
        onSuccess: () => {
          setAmount('');
          setEffectiveFromDate(format(new Date(), 'yyyy-MM-dd'));
        },
      },
    );
  };
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-3 p-4 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900">
        <p className="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">Set limit</p>
      ...
      {sorted.length > 0 && (
        <div className="flex flex-col gap-1">
          <p className="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wide mb-1">History</p>
          {sorted.map(entry => (
            <div
