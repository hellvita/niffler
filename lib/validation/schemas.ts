...
export const columnMappingSchema = z
  .object({
    ...
    categoryColumnIndexes: z
      .array(z.number().int().min(0))
      .min(1, 'Select at least one category column'),
  ...
  .refine((d) => !d.categoryColumnIndexes.includes(d.dateColumnIndex), {
    message: 'Date column cannot also be a category column',
    path: ['categoryColumnIndexes'],
  })
  .refine((d) => d.incomeColumnIndex !== d.dateColumnIndex, {
    message: 'Income column cannot be the same as the date column',
    path: ['incomeColumnIndex'],
  })
  .refine((d) => !d.categoryColumnIndexes.includes(d.incomeColumnIndex), {
    message: 'Income column cannot also be a category column',
    path: ['incomeColumnIndex'],
  });
